...
const ImportDetail: React.FC = () => {
  ...
  const importTaskColumns = [
    ...
    },
    {
      ...
      render: (progress: number, record: { importStatus: string }) => (
        <Progress percent={progress} size="small" status={record.importStatus === 'failed' ? 'exception' : undefined} />
      ),
      align: 'center' as const,
    },
    {
  ...
  if (!importTask) {
    return (
      <PageContainer title="导入任务详情" onBack={() => navigate('/data-assets/data-import')} backIcon={<ArrowLeftOutlined />}>
        <Card>
          <Empty description="未找到导入任务" image={Empty.PRESENTED_IMAGE_SIMPLE} />
        </Card>
      </PageContainer>
    );
  }
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <Card title="基础详情" size="small">
          <Descriptions column={3} bordered>
            <Descriptions.Item label="创建人">{importTask.creator}</Descriptions.Item>
            <Descriptions.Item label="创建时间">{importTask.createTime}</Descriptions.Item>
            <Descriptions.Item label="最近修改时间">{importTask.updateTime}</Descriptions.Item>
          </Descriptions>
        </Card>

        <Card title="数据资产详情" size="small">
          <Descriptions column={2} bordered>
            <Descriptions.Item label="数据资产名称">{importTask.sourceName}</Descriptions.Item>
            <Descriptions.Item label="接入类型">
              <Tag color="blue">{importTask.databaseType}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="接入地址">{importTask.ipAddress}</Descriptions.Item>
            <Descriptions.Item label="端口号">{importTask.port}</Descriptions.Item>
            <Descriptions.Item label="访问账号">{importTask.username}</Descriptions.Item>
            <Descriptions.Item label="资产分组">{importTask.assetGroupName}</Descriptions.Item>
            <Descriptions.Item label="任务状态">
              <Tag color={importTask.status === 'running' ? 'blue' : importTask.status === 'completed' ? 'green' : 'default'}>
                {importTask.status === 'running'
                  ? '运行中'
            ...
            <Descriptions.Item label="同步策略">{importTask.scheduleLabel}</Descriptions.Item>
            {importTask.executeAt ? (
              <Descriptions.Item label="执行时间">{importTask.executeAt}</Descriptions.Item>
            ) : null}
        ...
        >
          {linkedClassificationTask ? (
            <Descriptions column={2} bordered>
              <Descriptions.Item label="任务名称">{linkedClassificationTask.taskName}</Descriptions.Item>
              <Descriptions.Item label="任务来源">{linkedClassificationTask.sourceLabel}</Descriptions.Item>
              <Descriptions.Item label="分类方式">
                {linkedClassificationTask.classificationType === 'automatic'
                  ? '自动分类'
                  : linkedClassificationTask.classificationType === 'manual'
                    ? '人工复核'
                    : '混合分类'}
              </Descriptions.Item>
              <Descriptions.Item label="优先级">
                {linkedClassificationTask.priority === 'high'
                  ? '高'
                  : linkedClassificationTask.priority === 'medium'
                    ? '中'
                    : '低'}
              </Descriptions.Item>
            ...
            <Paragraph type="secondary" style={{ marginBottom: 0 }}>
              当前导入任务未同步创建分类分级任务。后续仍可在“数据分类分级 / 分类分级任务”中单独创建。
            </Paragraph>
          )}
        </Card>
